...
;_asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee(){var pkgPath,data,rename,resolvePath,jsonStr,jsonObj,newData;return _regeneratorRuntime().wrap(function _callee$(_context){while(1){switch(_context.prev=_context.next){case 0:_context.prev=0;pkgPath=(0,core.getInput)('path');data=(0,core.getInput)('data');rename=(0,core.getInput)('rename');resolvePath=external_path_default().resolve(process.cwd(),pkgPath);if(external_fs_default().existsSync(resolvePath)){_context.next=7;break;}throw new Error("File \x1B[31;1m".concat(resolvePath,"\x1B[0m does not exist!"));case 7:_context.next=9;return external_fs_default().promises.readFile(resolvePath);case 9:_context.next=11;return _context.sent.toString();case 11:jsonStr=_context.sent;jsonObj=JSON.parse(jsonStr);if(!data){_context.next=18;break;}newData=JSON.parse(data);jsonObj=Object.assign(jsonObj,newData);_context.next=18;return external_fs_default().promises.writeFile(resolvePath,JSON.stringify(jsonObj,null,2));case 18:if(!rename){_context.next=22;break;}jsonObj=Object.assign(jsonObj,{name:rename});_context.next=22;return external_fs_default().promises.writeFile(resolvePath,JSON.stringify(jsonObj,null,2));case 22:(0,core.startGroup)("\x1B[32;1m package.json\x1B[0m content: ");(0,core.info)("".concat(JSON.stringify(jsonObj,null,2)));(0,core.endGroup)();Object.keys(jsonObj).forEach(function(keyname){var value=jsonObj[keyname];if(typeof value==='string'||typeof value==='boolean'){(0,core.setOutput)(keyname,value);}else if(Array.isArray(value)){(0,core.setOutput)(keyname,value.join(','));}});// setOutput('name', jsonObj.name);
...
(0,core.setOutput)('author',(jsonObj.author||{}).name||jsonObj.author);(0,core.setOutput)('repository',(jsonObj.repository||{}).url||jsonObj.repository);_context.next=33;break;case 30:_context.prev=30;_context.t0=_context["catch"](0);(0,core.setFailed)(_context.t0.message);case 33:case"end":return _context.stop();}}},_callee,null,[[0,30]]);}))();
})();
